...
## Step 8 - Change Request 실행 순서

1. 사용자 원문과 추가 자료를 보존하고 placeholder·모호성·충돌을 표시한다.
2. 요구사항을 원자적 Change Item으로 나누고 의존성 순서를 확정한다.
3. HITL로 변경 내용·공개 범위·재현 환경을 승인받는다.
4. 승인 후에만 변경 전 Claude 검증 → Codex 최소 수정 → 동일 Claude 검증을 실행한다.
